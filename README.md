...
- отдельный ключ для Android;
- отдельный ключ администратора.

Не публикуйте эти ключи на сайте или в GitHub. Они создаются случайно и сохраняются только в `backend/runtime/online_config.json` на этом ПК.
...
### 5. Подключить Android-приложение
...
5. Введите HTTPS-адрес `https://….ts.net`, показанный `run_online_server.bat`.
6. Введите **ключ для Android** из того же окна.

Телефону добровольца не нужен аккаунт или приложение Tailscale.
...
Для теста без интернета запустите `backend/run_server.bat`. Телефон и ПК должны быть в одной Wi‑Fi-сети; адрес можно узнать командой `ipconfig`, например `http://192.168.1.25:8000`. Ключ этого режима: `tajik-stt-local`.
